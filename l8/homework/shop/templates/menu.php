...
<div class="leftblock">
    <header class="header">
        <nav class="navbar fixed-top navbar-expand-lg navbar-light bg-lightNone">
            ...
            <div class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav">
                    <li class="nav-item active">
                        <a class="nav-link" href="http://php1/l8/homework/shop/public/index.php">Главная <span class="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="guestbook.php">Отзывы</a>
                    </li>
                    <li class="nav-item">
                    ...
                    if (isset($_SESSION['login']) && isset($_SESSION['pass'])) {
                        echo "<li class='nav-item'><a class='nav-link' href='login.php?action=profile'>Кабинет</a></li>";
                        echo "<li class='nav-item'><a class='nav-link' href='login.php?action=logout'>Выйти ($_SESSION[login])</a></li>";

                    } else {
                        echo "<li class='nav-item'><a class='nav-link' href='login.php'><u>Войти</u></a></li>";
                        echo "<li class='nav-item'><a class='nav-link' href='reg.php'><u>Регистрация</u></a></li>";
                    }
            ...
            <button class="btn btn-outline-success btn-primary basketInfoOut" type="button"
                    onclick='renderBasketModal()'
                    data-toggle="modal" data-target="#bascketModal">
                <?php
                if ($countGoodsOrder[0]) {
                    echo('<strong>Корзина</strong>' . '<br>' . '<strong>' . $countGoodsOrder[0] . '</strong>');
                } else {
                    echo('<strong>Корзина</strong>' . '<br>' . '<strong>товаров 0</strong>');
                };
                ?>
            </button>
        </nav>
        <!-- Modal -->
        <div class="modal fade" id="bascketModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
             aria-hidden="true">
